...
def add_to_growth():
    ...
    try:
        with engine.connect() as conn:
            check_query = text("SELECT COUNT(1) FROM growth WHERE seccode = :sec_code")
            count = conn.execute(check_query, {"sec_code": sec_code}).scalar()
            if count > 0:
                return jsonify({"message": f"Sec Code {sec_code} is already in the growth."}), 200
            ...
            query = text("""
                ...
                LIMIT 1
            """)
            company_data = pd.read_sql(query, conn, params={"sec_code": sec_code})

            if company_data.empty:
                return jsonify({"message": "Company not found"}), 404

        insert_query = text("""
            INSERT INTO growth (seccode, companyname, fiscalyearend, quarter, growth_percentage,
